...
def withdraw_money(person, money):
    if person['money'] >= money:
        person['money'] -= money
        return 'Вы сняли {} рублей. Остаток по карте: {} рублей.'.format(money, round(person['money'],2))
    else:
    ...
    if choice == '1':
        ...
        try:
            ...
            if count > 0:
                ...
                print('Сумма к снятию введена некорректно')
        except ValueError:
            print('Сумма к снятию введена некорректно')


def start():
    card_and_pin_template = '[\d]{16} [\d]{4}$'

    card_and_pin_input = input('Введите номер карты и пин код через пробел:')
    while not re.match(card_and_pin_template, card_and_pin_input):
        print('Ошибка ввода данных!')
        card_and_pin_input = input('Введите номер карты и пин код через пробел:')

    card_number, pin_code = card_and_pin_input.split()
